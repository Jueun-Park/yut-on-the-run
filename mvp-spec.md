...
#### 5.1.1 확률 분포(정통에 가까운 가중치)
던지기 결과는 다음 가중치 테이블에 따라 결정된다:
...
- 누적 확률: 도 [0, 0.3), 개 [0.3, 0.6), 걸 [0.6, 0.85), 윷 [0.85, 0.95), 모 [0.95, 1.0)
- RNG 샘플링: `random() ∈ [0, 1)`을 생성하여 누적 구간에 따라 결과 선택
- 예: `random() = 0.42` → 0.42 ∈ [0.3, 0.6) → 개(2)
...
### 5.4 완주(오버슛) 규칙
- `O20`에 **도착만 해서는 완주되지 않음**.
- `O20`에서 출발하여 **1 step 이상 소비하려 할 때** 즉시 `FINISHED` 상태로 전환된다.
- 예: 
  - O19에서 도(1) 사용 → O20 도착, 아직 완주 아님.
  - O20에서 도(1) 사용 → 오버슛으로 즉시 완주(FINISHED).
  - O19에서 개(2) 사용 → O20을 지나므로 즉시 완주(FINISHED).
